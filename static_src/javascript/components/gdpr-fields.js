class GdprFields {
    ...
    submitted(event) {
        ...
        const fundraisingPref = document.getElementById('id_prefs_volunteering');

        const consentWrapper = document.getElementById('donate__contact-prefs');

        if (
            (phonePrefs.checked || emailPrefs.checked || smsPrefs.checked) && (!volunteeringPref.checked && !fundraisingPref.checked)
        ) {
            ...
            if (!this.errorAdded) {
                ...
                error.innerText = 'Please select at least one reason we can contact you for, since you\x27ve selected a contact method.';

                consentWrapper.appendChild(error);
                this.errorAdded = true;
            }
            consentWrapper.parentNode.parentNode.scrollIntoView({ behavior: 'smooth' });
        } else {
            this.submit();
        }
    }
